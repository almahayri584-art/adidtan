...
function renderDepoList() {
  depoList.innerHTML = '';
  if (!depoData.length) {
    depoList.innerHTML = '<div class="empty">Henüz kayıt yok.</div>';
    return;
  }
  depoData.forEach(u => {
    ...
    (u.texts || []).forEach(t => {
      ...
      copyB.addEventListener('click', () => copyPlain(t, copyB));
